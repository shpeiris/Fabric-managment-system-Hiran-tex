...
export default function Feedback() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    console.log('Feedback submitted:', { ...feedback, overallRating: rating })
  }

  const RatingStars = ({ value, onChange, label }) => (
    <div style={{ marginBottom: '20px' }}>
      <label style={{ display: 'block', fontSize: '14px', color: '#6b7280', marginBottom: '10px', fontWeight: '500' }}>
        {label}
      </label>
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ marginBottom: '30px' }}>
              <RatingStars
                value={feedback.orderExperience}
                onChange={(val) => setFeedback({ ...feedback, orderExperience: val })}
                label="Order Experience"
              />
              <RatingStars
                value={feedback.fabricQuality}
                onChange={(val) => setFeedback({ ...feedback, fabricQuality: val })}
                label="Fabric Quality"
              />
              <RatingStars
                value={feedback.delivery}
                onChange={(val) => setFeedback({ ...feedback, delivery: val })}
                label="Delivery Service"
              />
              <RatingStars
                value={feedback.customerService}
                onChange={(val) => setFeedback({ ...feedback, customerService: val })}
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
              <div>
                ...
                <p style={{ fontSize: '14px', color: '#6b7280' }}>Helps us improve our products and services</p>
              </div>
              <div>
                <div style={{ fontSize: '24px', marginBottom: '5px' }}>🎯</div>
                <p style={{ fontSize: '14px', color: '#6b7280' }}>Guides our future fabric selections</p>
              </div>
              <div>
                <div style={{ fontSize: '24px', marginBottom: '5px' }}>🤝</div>
                <p style={{ fontSize: '14px', color: '#6b7280' }}>Helps other customers make informed decisions</p>
              </div>
              <div>
                <div style={{ fontSize: '24px', marginBottom: '5px' }}>💡</div>
                <p style={{ fontSize: '14px', color: '#6b7280' }}>Shapes our customer experience</p>
              </div>
            </div>
          </div>
